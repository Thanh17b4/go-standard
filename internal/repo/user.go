...
func (u *User) Update(userId int64, user model.User) (*model.User, error) {
	fmt.Println("user: ", userId)
	_, err := u.db.Exec("UPDATE `users` SET `name`= ? , `email`= ?, `address`= ? WHERE `id` = ?", user.Name, user.Email, user.Address.String, user.ID)
	if err != nil {
